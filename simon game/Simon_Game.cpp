...
int main() {
    ...
    auto lastButtonPressTime = high_resolution_clock::now();

    const std::chrono::seconds gameTimeoutDuration(5);
    ...
    for (int i = 0; i < static_cast<int>(ColorEnum::COUNT); ++i) {
        ...
        {
            std::cout << " iam here"; 
            if (game.hasGameTimedOut(lastButtonPressTime, gameTimeoutDuration))
            {
                game.gameOver(window, gui);
                break;
            }

            if (colorButtons[0]->isFocused() || colorButtons[1]->isFocused() || colorButtons[2]->isFocused() || colorButtons[3]->isFocused())
                break;
          ...
          colorButtons[i]->onClick([&, i]() {
              ...
              lastButtonPressTime = high_resolution_clock::now(); 
            });
